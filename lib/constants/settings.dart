class SettingsConstants {
  static const Duration duration150 = Duration(milliseconds: 150);
  static const Duration duration300 = Duration(milliseconds: 300);
  static const Duration duration500 = Duration(milliseconds: 500);
  static const Duration inactivityDuration = Duration(minutes: 10);

  static const Duration btnCoolDownDuration = Duration(seconds: 10);

  static const String dayMonth = 'dd/MM';
  static const String hourMinute = 'HH:mm';
  static const String yearMonthDay = 'yyyy-MM-dd';
  static const String yearMonthDayHourMinuteSecond = 'yyyy-MM-dd HH:mm:ss';
}
